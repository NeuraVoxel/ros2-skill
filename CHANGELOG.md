...
### Proximity Sensor Discovery (Wave 6)

- **Rule 25 / AGENTS.md Rule 39** — before any motion > 5 s, auto-discover `LaserScan`, `Range`, and `PointCloud2` topics; report found sensors in motion output; skip silently if none found. Full real-time obstacle avoidance documented as out-of-scope (platform-specific) with reference thresholds (< 0.5 m → reduce speed; < 0.2 m → estop).
